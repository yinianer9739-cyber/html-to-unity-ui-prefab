...
弹窗类流程默认做成独立 `UIView`，通过项目 UI 管理器打开和关闭，不嵌进发起它的当前 View。适用范围包括普通弹窗、侧边栏奖励界面、帮助/关于/个人信息面板、确认框、平台入口弹层，以及类似“点击后打开一个界面”的流程。

只有当 UI 明确属于当前屏幕的强耦合同屏状态时，才考虑同一个 View 内部面板，例如底部四个按钮切换、Tab 内容、分段模式、需要原地常驻切换的模式面板。遇到这类情况且推荐同 View 面板但需求不够明确时，先说明推荐理由，再询问用户是否按这种结构走。

弹窗类流程不需要询问是否新建 View，默认新建独立 `UIView`。不要把这条规则理解成 prefab 层级必须扁平；它约束的是 View 归属和运行时 UI 流程，不是 Unity prefab 内部 GameObject 是否有父子层级。
