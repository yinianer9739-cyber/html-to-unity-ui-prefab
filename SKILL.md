...
6. Generate or recreate `UIXXXView` and needed `UIXXXItem` prefabs under `Assets/Resources/ui/`.
7. Attach matching `UIXXXView.cs` when it exists, use UGUI components, and preserve project naming rules.
8. Emit a generation report listing created files, unsupported CSS, inferred nodes, missing assets, and manual checks.
