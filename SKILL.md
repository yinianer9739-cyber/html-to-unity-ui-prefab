...
- Respect existing framework loader boundaries: prefabs, views, and atlases normally go through `MiniFrameWork.AssetManager`; UI atlas sprites for documented dynamic business states may go through `MiniFrameWork.UIManager.GetSprite/TryGetSprite`. Generated `ConfigXXXEntry` JSON follows the project rule of loading `TextAsset` data from `Resources/config/ConfigXXXEntry`; do not add new `AssetManager` APIs for it unless the framework owner approves. Do not create parallel business-side `Resources.Load` wrappers, add ad-hoc `LoadSprite` APIs, or scatter direct `Resources.Load` calls in business code.
...
3. Write the element evidence plan and structured prefab spec before conversion or generation decisions.
...
5. Confirm TexturePacker is installed before asset or atlas work. Stop if missing.
6. Only after source review and planning, render the HTML in a browser and collect `getBoundingClientRect()` plus computed styles for those states.
7. Use screenshots only after source review as visual verification, mismatch evidence, or gap evidence.
8. Convert the source-backed and browser-verified layout/state logic into a structured UI description.
